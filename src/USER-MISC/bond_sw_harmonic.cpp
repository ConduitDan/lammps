...
BondStillingerWeberHarmonic::BondStillingerWeberHarmonic(LAMMPS *lmp)
  : Bond(lmp),
  k{nullptr},
  k_harmonic{nullptr},
  lmin{nullptr},
...
BondStillingerWeberHarmonic::~BondStillingerWeberHarmonic()
{
...
void BondStillingerWeberHarmonic::compute(int eflag, int vflag)
{
  int i1,i2,n,type;
  ...
  for (n = 0; n < nbondlist; n++) {
    ...
    l0 = lmin[type];
    l3 = lmax[type];
    double eq_length = equilibrium_distance(type);
    l1 = l0 + delta[type];
    l2 = l3 - delta[type];    
...
void BondStillingerWeberHarmonic::allocate()
{
  allocated = 1;
...
void BondStillingerWeberHarmonic::init_style()
{
...
void BondStillingerWeberHarmonic::write_restart(FILE *fp)
{
...
void BondStillingerWeberHarmonic::read_restart(FILE *fp)
{
  allocate();
...
double BondStillingerWeberHarmonic::single(int type, double rsq, int /*i*/, int /*j*/,
                        double &fforce)
{
  double a, b, c, u;
  ...
  fforce -= k_harmonic[type] * (r-eq_length);
  eng += 0.5 * k_harmonic[type] * (r-eq_length) * (r-eq_length);
  return eng;
}

/* ---------------------------------------------------------------------- */

void *BondStillingerWeberHarmonic::extract(const char *str, int &dim)
{
  dim = 1;
